...
class Project < ActiveRecord::Base
  ...
  scope :expiring_in_day, -> { not_expired.where(expires_at: Time.current.. 1.day.from_now, day_before_mail_sent: false) }
  scope :expiring_in_two_days, -> { not_expired.where(expires_at: Time.current.. 2.day.from_now, two_days_before_mail_sent: false) }
  ...
  scope :goal_met_not_emailed, -> { select(&:reached_goal_and_not_emailed?) }
  ...
  validates_numericality_of :online_days, less_than_or_equal_to: 120, greater_than: 0,
    if: ->(p){ p.online_days.present? && ( p.online_days_was.nil? || p.online_days_was <= 120 ) }
  ...
  def backers
    @backers ||= User.where(id: contributions.confirmed.not_anonymous.pluck(:user_id).uniq)
  end
  ...
  def reached_goal?
    pledged.to_f >= goal.to_f
  end

  def reached_goal_and_not_emailed?
    (reached_goal?) && (emailed_about_goal_met == false)
  end
